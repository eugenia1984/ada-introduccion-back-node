...
Hacemos el ejercicio 12 de la guia.
...
- Para ver todas las llaves de un objeto:

```JavaScript
const todasLasLlaves = Object.keys(user);
```

-> Entre los parentesis le paso el objeto del cual quiero ver las llaves

-> Se ven todas las key en un array


- Para ver todos los values es similar:

```JavaScript
const todasLosVAlores = Objects.values(user);
```

- Para ver ambos:

```JavaScript
const todosLosAtributos = Object.entries(user);
```
...
### Hacemos uno de los ejercicios de la guia de objetos
